...
- 🖥️ **Flask 기반 웹 UI**: 브라우저에서 바로 크롤러 사용 가능
- 🔍 **검색어 입력 폼**: 직관적인 검색 인터페이스
- 📥 **결과 다운로드**: Excel/CSV 형식으로 다운로드 지원
- 📊 **실시간 결과 표시**: 검색 결과를 테이블 형태로 즉시 확인
- 📋 **검색 이력 관리**: 이전 검색 결과 확인 및 재다운로드
...
```bash
# 웹 인터페이스 예시 실행
python web_interface_example.py
```
